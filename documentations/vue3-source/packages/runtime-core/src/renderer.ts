...
function baseCreateRenderer(
  ...
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    // 相当于 vue2 中 componentUpdate
    // effect(fn) 添加副作用，响应式数据发生变化时，fn会执行
    // 内部 render 函数的执行会触发依赖收集
    // 这个机制消除了 watcher
  ...
  const render: RootRenderFunction = (vnode, container) => {
    if (vnode == null) {
      ...
      // patch 方法（之前容器中的虚拟DOM：_vnode 首次为空， 现在的 vnode， id 容器）
